        ...
        <div class="courses-grid">
          <mat-card class="course-card" *ngFor="let course of popularCourses">
            <div class="course-image">
              <img *ngIf="course.thumbnailUrl; else popularIcon" [src]="course.thumbnailUrl"
                [alt]="course.name + ' course cover'" loading="lazy" />
              <ng-template #popularIcon><mat-icon>{{ course.icon }}</mat-icon></ng-template>
              <span class="popular-label"><mat-icon>local_fire_department</mat-icon> Popular now</span>
            </div>
            <mat-card-content>
              <h3>{{ course.name }}</h3>
...
export class HomepageComponent implements OnInit {
      ...
      next: (courses) => {
        if (courses.length) {
          this.popularCourses = [...courses]
            .sort((a, b) => ((b.studentsEnrolled || 0) - (a.studentsEnrolled || 0)) || ((b.rating || 0) - (a.rating || 0)))
  ...
  learnMore(course: any): void {
    this.router.navigate(course.id ? ["/courses", course.id] : ["/courses"]);
  }
