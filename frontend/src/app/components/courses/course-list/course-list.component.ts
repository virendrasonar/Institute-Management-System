...
@Component({
  ...
  template: `
    <div class="page-container">
      ...
      <div *ngIf="!loading && courses.length === 0" class="empty-state">
        ...
        <p>{{ auth.isAuthenticated() ? "Start by creating your first course for students." : "New courses are coming soon." }}</p>

        <button *ngIf="auth.isAuthenticated()" mat-raised-button color="primary" routerLink="/courses/new">
          <mat-icon>add</mat-icon>
          Create Course
        </button>
      </div>
      ...
      <div class="card-grid" *ngIf="!loading">
        ...
        <mat-card class="course-card" *ngFor="let course of courses">
          ...
          <div class="course-image">
            <img *ngIf="course.thumbnailUrl; else courseIcon" [src]="course.thumbnailUrl"
              [alt]="course.name + ' course cover'" loading="lazy" />
            <ng-template #courseIcon><mat-icon>{{ course.icon || "school" }}</mat-icon></ng-template>
  ...
  styles: [
    `
      ...
      .popular-badge { position: absolute; left: 14px; bottom: 14px; display: flex; align-items: center; gap: 4px; padding: 7px 11px; border-radius: 999px; background: rgba(17,24,39,.86); color: #fff; font-size: 12px; font-weight: 700; backdrop-filter: blur(8px); }
      .popular-badge mat-icon { width: 16px !important; height: 16px !important; font-size: 16px !important; color: #fbbf24; }
