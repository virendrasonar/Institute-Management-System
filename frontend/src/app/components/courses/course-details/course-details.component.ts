...
@Component({
        ...
        <div class="content-grid">
          ...
          <aside>
            <mat-card class="enroll-card">
              <div class="price">{{ item.price ? (item.price | currency:'INR':'symbol':'1.0-0') : 'Enrollment open' }}</div>
  ...
  styles: [`
    ...
    aside{min-width:0}.enroll-card{position:sticky;top:90px;width:100%;background:#fff}.price{font-weight:800;font-size:25px;color:#5b21b6;margin-bottom:9px}.enroll-card>p{color:#666;line-height:1.55}.enroll-card form{display:grid;gap:6px;margin-top:22px}.enroll-card mat-form-field{width:100%}.enroll-card button{height:52px;border-radius:11px;font-weight:700}.enroll-card button mat-spinner{display:inline-block;margin-right:8px}.enroll-card small{display:flex;align-items:center;justify-content:center;gap:5px;margin-top:16px;color:#777;text-align:center}.enroll-card small mat-icon{font-size:15px;width:15px;height:15px}
    @media(max-width:900px){.page-shell{width:min(100% - 30px,760px);padding-top:26px}.hero{grid-template-columns:1fr}.thumbnail{min-height:260px}.hero-copy{padding:32px}.content-grid{grid-template-columns:1fr}.enroll-card{position:static}.content-grid aside{order:-1}}
...
export class CourseDetailsComponent implements OnInit {
  ...
  enroll(): void {
    ...
    const { name, email } = this.enrollmentForm.getRawValue();
    this.courseService.enroll(this.course.id, name.trim(), email.trim()).subscribe({
      next: (result) => {
