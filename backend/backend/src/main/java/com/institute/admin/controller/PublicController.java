...
import com.institute.admin.dto.EnrollmentRequest;
import com.institute.admin.dto.EnrollmentResponse;
import com.institute.admin.dto.LearningCourseResponse;
import com.institute.admin.dto.PublicCourseResponse;
...
public class PublicController {
    ...
    @Autowired
    public PublicController(AdminService adminService, EnrollmentService enrollmentService) {
    ...
    public ResponseEntity<List<PublicCourseResponse>> getAllCourses() {
        try {
            List<PublicCourseResponse> courses = adminService.getAllCourses().stream()
    ...
    public ResponseEntity<PublicCourseResponse> getCourseById(@PathVariable Long id) {
        try {
            Optional<Course> course = adminService.getCourseById(id);
            return course.map(PublicCourseResponse::from).map(ResponseEntity::ok)
    ...
    @GetMapping("/learning/{accessToken}")
    public ResponseEntity<LearningCourseResponse> getLearningCourse(@PathVariable String accessToken) {
        try {
            return ResponseEntity.ok(LearningCourseResponse.from(enrollmentService.getByAccessToken(accessToken)));
        } catch (IllegalArgumentException exception) {
